...
def extract_text(file_bytes: bytes, filename: str) -> str:
    ...
    if name.endswith((".zip", ".xip")):
        return _archive_to_text(file_bytes, filename)
...
def _archive_to_text(file_bytes: bytes, filename: str) -> str:
    """Extract supported text assets from a zip-based archive."""

    try:
        archive = ZipFile(BytesIO(file_bytes))
    except BadZipFile as exc:
        raise HTTPException(status_code=415, detail="Unsupported archive format") from exc

    try:
        members = [info for info in archive.infolist() if not info.is_dir()]
    except RuntimeError as exc:  # pragma: no cover - rare zip errors
        raise HTTPException(status_code=422, detail="Unable to read archive") from exc
    ...
    for info in members:
        ...
        except Exception as exc:  # noqa: BLE001 - zipfile may raise various errors
            logger.warning("Failed to read archive member", member=member_name, error=str(exc))
            continue

        try:
            member_text = extract_text(data, member_name)
        ...
        cleaned = member_text.strip()
        if cleaned and _is_readable_text(cleaned):
            snippet = f"# File: {member_name}\n{cleaned}"
            aggregated.append(snippet)

    if not aggregated:
        raise HTTPException(status_code=422, detail="Archive did not contain readable documents")

    return "\n\n".join(aggregated)


def _is_readable_text(text: str) -> bool:
    """Heuristic to skip binary-like payloads that survived decoding."""
