...
def resolve_space(space_key: str | None = None, space_id: str | None = None) -> Dict[str, Any]:
    ...
    if not space_key and not space_id:
        _tool_args_invalid("space_key or space_id required", details={"missing": ["space_key", "space_id"]})
    ...
    with sync_client(timeout=30) as client:
        ...
        if not results:
            _tool_args_invalid("Confluence space not found", details={"space_key": space_key})
...
def discover_default_space() -> Dict[str, Any]:
    ...
    with sync_client(timeout=30) as client:
        ...
        results = data.get("results", [])
        if not results:
            _tool_args_invalid("No Confluence spaces available")
        item = results[0]
        return {
            "id": _to_str_or_none(item.get("id")),
...
def create_page(
    *,
    ...
    if not space_id_str or not space_key_str:
        _tool_args_invalid("space_key or space_id required", details={"space_key": space_key, "space_id": space_id})
